...
class StockPredictor(object):
    ...
    def load_test(self, test_data):
        ...
        self._test_data_pandas = test_data
        self._test_data = data[['open','close','high','low']].to_numpy()
    ...
    def _split_train_test_data(self, data, test_size):
        ...
        self._train_data_pandas = _train_data
        self._test_data_pandas = test_data
        self._train_data = _train_data[['open','close','high','low']].to_numpy()
        self._test_data = test_data[['open','close','high','low']].to_numpy()
    ...
    def predict_close_prices_for_days(self, days, recent=False, with_plot=False, save_plot=False):
        ...
        if days > self._test_data.shape[0]:
            days = self._test_data.shape[0]
        elif recent: # set the test data to the most recent data
            self._test_data = self._test_data[self._test_data.shape[0]-days:,:]
        ...
        test_data = self._test_data_pandas[0: days]
